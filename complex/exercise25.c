...
void getk(struct BTreeNode *myNode, int *count, int index) {
  int i;
  if (myNode) {
    for (i = 0; i < myNode->count; i++) {
      getk(myNode->linker[i], count, index);
      (*count)++;
      if((*count) == index){
          printf("%d\n", myNode->item[i + 1]);
      }
...
void removek(struct BTreeNode *myNode, int *count, int index) {
  int i;
  if (myNode) {
    for (i = 0; i < myNode->count; i++) {
      getk(myNode->linker[i], count, index);
...
int main() {
  ...
  while(1){
    ...
    } else if(strcmp(input_str, "get") == 0){

      scanf("%d" ,&val);
      printf("get(%d) = " ,val);
      searching(val, &ch, root);

    } else if(strcmp(input_str, "getk") == 0){

      scanf("%d" ,&val);
      printf("getk(%d) = " ,val);
      getk(root, &counter, val);
      if(val > counter || val <= 0){
        printf("not found\n");
      }
    } else if(strcmp(input_str, "remove") == 0){

      scanf("%d" ,&val);
      printf("remove(%d) = " ,val);
      delete(val, root);
    } else if(strcmp(input_str, "removek") == 0){
      
      scanf("%d" ,&val);
      printf("removek(%d) = " ,val);
      removek(root, &counter, val);
      if(val > counter || val <= 0){
        printf("not found\n");
      }
    } else {
